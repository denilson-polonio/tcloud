...
function deleteUser(id) {
  ...
  const staged = db.prepare('SELECT staged_path FROM files WHERE owner_id = ? AND staged_path IS NOT NULL').all(id).map((r) => r.staged_path);
  db.transaction(() => {
    const now = Date.now();
    const ins = db.prepare('INSERT INTO pending_deletions (message_id, attempts, created_at) VALUES (?, 0, ?)');
    for (const c of db.prepare('SELECT message_id FROM chunks WHERE message_id IS NOT NULL AND file_id IN (SELECT id FROM files WHERE owner_id = ?)').all(id)) ins.run(c.message_id, now);
    db.prepare('DELETE FROM users WHERE id = ?').run(id);
  })();
  for (const p of staged) { try { fs.unlinkSync(p); } catch (_) {} }
}
