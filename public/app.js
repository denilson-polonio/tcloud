...
function parseDuration(str) { if (!str) return null; const mult = { s: 1, m: 60, h: 3600, d: 86400, w: 604800, y: 31536000 }; const re = /(\d+)\s*([smhdwy])/gi; let m, total = 0, found = false; while ((m = re.exec(str)) !== null) { total += parseInt(m[1], 10) * mult[m[2].toLowerCase()]; found = true; } return found ? total : null; }
...
$('#su-go').addEventListener('click', async () => {
  ...
  const shp = $('#su-shareport') && parseInt($('#su-shareport').value, 10); if (shp > 0) body.sharePort = shp;
  const shu = $('#su-shareurl') && $('#su-shareurl').value.trim(); if (shu) body.sharePublicUrl = shu;
...
$('#nav-activity').addEventListener('click', () => openActivity());
...
let _logLimit = 50;
let _actSeq = 0;
function _actStop() { if (_actTimer) { clearInterval(_actTimer); _actTimer = null; } }
function setNav() { _actStop(); for (const [id, ty] of [['nav-root', 'folder'], ['nav-starred', 'starred'], ['nav-tdrop', 'tdrop'], ['nav-shares', 'shares'], ['nav-activity', 'activity'], ['nav-sync', 'sync'], ['nav-admin', 'admin']]) $('#' + id).classList.toggle('active', view.type === ty); document.querySelectorAll('#ext-nav .ext-injected').forEach((el) => el.classList.toggle('active', view.type === 'ext' && view.id === el.dataset.ext)); const chromeless = view.type === 'admin'; $('#view-toggle').classList.toggle('hidden', chromeless); const _sb = document.querySelector('.search'); if (_sb) _sb.classList.toggle('hidden', chromeless || view.type === 'sync'); }
...
async function adminExtensions() {
  ...
  const nt = canS ? await api('/notifications').catch(() => ({})) : {};
  let html = '';
  if (canS) {
  ...
  if (canS) html += `<div class="section-h" style="margin-top:26px">\ud83d\udd14 ${t('Notifications')}</div><div class="setting-card"><div class="setting-desc" style="margin-bottom:10px">${t('Get a Telegram message from the bot when these happen. Requires your Telegram account to be linked (set your Telegram ID in the Telegram tab).')}</div><div class="setting-row"><div><div class="setting-title">${t('Upload finished')}</div><div class="setting-desc">${t('When a file finishes uploading to Telegram.')}</div></div><label class="switch"><input type="checkbox" id="nt-upload_done" ${nt.upload_done ? 'checked' : ''}/><span class="slider"></span></label></div><div class="setting-row"><div><div class="setting-title">${t('Upload failed')}</div><div class="setting-desc">${t('When an upload errors out.')}</div></div><label class="switch"><input type="checkbox" id="nt-upload_failed" ${nt.upload_failed ? 'checked' : ''}/><span class="slider"></span></label></div><div class="setting-row"><div><div class="setting-title">${t('Share downloaded')}</div><div class="setting-desc">${t('When someone downloads one of your shared links.')}</div></div><label class="switch"><input type="checkbox" id="nt-share_download" ${nt.share_download ? 'checked' : ''}/><span class="slider"></span></label></div><div class="setting-row"><div><div class="setting-title">${t('Guest connected')}</div><div class="setting-desc">${t('When a guest links to your bot for the first time.')}</div></div><label class="switch"><input type="checkbox" id="nt-guest_join" ${nt.guest_join ? 'checked' : ''}/><span class="slider"></span></label></div><div class="modal-row"><button class="modal-btn primary" id="nt-save">${t('Save settings')}</button></div><div id="nt-result"></div></div>`;
  ...
  const _ntSave = $('#nt-save'); if (_ntSave) _ntSave.onclick = async () => { const j = { upload_done: $('#nt-upload_done').checked, upload_failed: $('#nt-upload_failed').checked, share_download: $('#nt-share_download').checked, guest_join: $('#nt-guest_join').checked }; const btn = _ntSave; const orig = btn.textContent; btn.disabled = true; btn.textContent = t('Saving\u2026'); $('#nt-result').innerHTML = ''; try { await api('/notifications', { method: 'PATCH', json: j }); btn.classList.add('saved-ok'); btn.textContent = '\u2705 ' + t('Saved'); setTimeout(() => { btn.classList.remove('saved-ok'); btn.textContent = orig; btn.disabled = false; }, 1800); } catch (e) { btn.disabled = false; btn.textContent = orig; $('#nt-result').innerHTML = `<div class="login-error">${esc(e.message)}</div>`; } };
}
...
function renderFolderNote(note, folderId) {
  ...
  if (note) {
    ...
    div.innerHTML = `<div class="fn-head"><button class="tool fn-edit" title="${t('Edit note')}">${I.rn}</button></div><div class="fn-body"></div>`;
    div.querySelector('.fn-body').innerHTML = mdToHtml(note);
...
function reload() { if (view.type === 'starred') openStarred(); else if (view.type === 'activity') openActivity(); else if (view.type === 'shares') openShares(); else if (view.type === 'admin') openAdmin(); else if (view.type === 'tdrop') openTDrop(); else if (view.type === 'sync') openFolder(view.id, true); else openFolder(view.id); }
...
window.addEventListener('dragenter', (e) => { if (isFileDrag(e)) e.preventDefault(); });
window.addEventListener('dragover', (e) => { if (!isFileDrag(e)) return; e.preventDefault(); if (dragHideTimer) { clearTimeout(dragHideTimer); dragHideTimer = null; } showDrop(); });
window.addEventListener('dragleave', (e) => { if (!isFileDrag(e)) return; if (dragHideTimer) clearTimeout(dragHideTimer); dragHideTimer = setTimeout(hideDrop, 80); });
window.addEventListener('drop', (e) => { if (dragHideTimer) { clearTimeout(dragHideTimer); dragHideTimer = null; } hideDrop(); if (isFileDrag(e) && can('upload') && (view.type === 'folder' || view.type === 'sync') && e.dataTransfer.files.length) { e.preventDefault(); const f = Array.from(e.dataTransfer.files); const dest = curFolder(); enqueueUpload(() => uploadFiles(f, dest)); } });
...
function shareModal(type, res, existing) {
  ...
  const expirySel = (editing ? `<option value="-1" selected>${t('Leave unchanged')}</option>` : '') + EXPIRY_OPTS.map((o) => `<option value="${o.v}" ${!editing && o.v === 0 ? 'selected' : ''}>${t(o.label)}</option>`).join(''); const expCustom = `<option value="custom">${t('Custom\u2026')}</option>`;
  modal(`<h2>${editing ? t('Edit share') : t('Share')} "${esc(res.name)}"</h2><label>${t('Label (optional)')}</label><input type="text" id="sh-label" value="${esc((existing && existing.label) || '')}" />${isFolder
    ...
    `<div class="grid2"><div><label>${t('Expiry')}</label><select id="sh-exp">${expirySel}${expCustom}</select><input type="text" id="sh-exp-custom" class="hidden" placeholder="${t('e.g. 1h 30m, 2d, 1y')}" style="margin-top:8px;width:100%;box-sizing:border-box" /></div><div><label>${t('Download limit')}</label><input type="number" id="sh-max" min="0" placeholder="${t('unlimited')}" value="${existing && existing.max_downloads ? existing.max_downloads : ''}" /></div></div>` +
  ...
  const _expSel = $('#sh-exp'); if (_expSel) { const _ec = $('#sh-exp-custom'); const _se = () => { if (_ec) _ec.classList.toggle('hidden', _expSel.value !== 'custom'); }; _expSel.onchange = _se; _se(); }
  if (isFolder) {
    const hints = {
  ...
  $('#sh-ok').onclick = async () => {
    ...
    const expV = $('#sh-exp').value; if (expV === 'custom') { const _cs = parseDuration($('#sh-exp-custom').value); if (!_cs) { $('#sh-result').innerHTML = `<div class="login-error">${t('Enter a valid duration, e.g. 1h 30m, 2d, 1y.')}</div>`; return; } body.expiresIn = _cs; } else if (expV !== '-1') body.expiresIn = parseInt(expV, 10);
...
async function openActivity() {
  view = { type: 'activity', id: null }; setNav(); renderTree(); clearSel(); _logLimit = 50;
  $('#breadcrumb').innerHTML = `<span class="crumb current">${t('Activity')}</span>`;
  const draw = async () => {
    if (view.type !== 'activity') { _actStop(); return; }
    const mySeq = ++_actSeq;
    let d; try { d = await api('/activity'); } catch (_) { return; }
    const showLog = can('manageSettings');
    let log = null; if (showLog) { try { log = await api('/activity/log?limit=' + _logLimit); } catch (_) { log = null; } }
    const c = $('#content'); if (!c) { _actStop(); return; }
    let h = '';
    if (d.interrupted && d.interrupted.length) {
      h += `<div class="section-h">${t('Interrupted by a crash')}</div>`;
      h += d.interrupted.map((x) => `<div class="setting-card act-card"><div class="act-row"><div><div class="setting-title">${esc(x.name)}</div><div class="setting-desc">${t('Was not finished before the last restart.')} · ${fmtSize(x.uploaded || 0)} / ${fmtSize(x.size)}${x.resumable ? '' : ' · ' + t('source gone — cannot resume')}</div></div><div style="display:flex;gap:8px;flex:none">${x.resumable ? `<button class="modal-btn primary" data-resume="${esc(x.id)}">${t('Resume')}</button>` : ''}<button class="modal-btn danger" data-abandon="${esc(x.id)}">${t('Abandon')}</button></div></div></div>`).join('');
    }
    h += `<div class="section-h"${d.interrupted && d.interrupted.length ? ' style="margin-top:22px"' : ''}>${t('Running now')}</div>`;
    if (!d.active.length) h += `<div class="empty"><div class="empty-mark">\u25c7</div>${t('Nothing is running right now.')}</div>`;
    else h += d.active.map((a) => { const pct = a.total ? Math.min(100, Math.round(a.done / a.total * 100)) : 0; const where = a.target === 'telegram' ? t('Uploading to Telegram') : (a.target === 'machine' ? t('Saving to disk') : t('Working')); return `<div class="setting-card act-card"><div class="act-head"><span class="setting-title">${esc(a.name)}</span><span class="su-note">${where} · ${pct}%${a.total ? ' · ' + fmtSize(a.done) + ' / ' + fmtSize(a.total) : ''}</span></div><div class="prog"><div class="prog-fill" style="width:${pct}%"></div></div></div>`; }).join('');
    if (showLog && log && log.events) {
      h += `<div class="section-h ev-head" style="margin-top:22px"><span>${t('Activity log')} <span class="su-note">(${log.total})</span></span>${log.events.length ? `<button class="mini danger" id="ev-clear">${t('Clear log')}</button>` : ''}</div>`;
      if (!log.events.length) h += `<div class="empty"><div class="empty-mark">\u25c7</div>${t('No activity recorded yet.')}</div>`;
      else {
        h += `<div class="setting-card act-card"><div class="ev-log">` + log.events.map((e) => {
          const k = String(e.kind || 'other').replace(/[^a-z]/gi, '');
          const when = e.ts ? new Date(e.ts).toLocaleString() : '';
          const det = e.detail ? `<span class="ev-detail">${esc(e.detail)}</span>` : '';
          const ip = e.ip ? `<span class="ev-ip">${esc(e.ip)}</span>` : '';
          return `<div class="ev-row"><span class="ev-badge ev-${k}">${esc(String(e.kind || 'other'))}</span><span class="ev-who">${esc(e.actor || '')}</span><span class="ev-act">${esc(e.action || '')}</span>${det}<span class="ev-meta">${ip}<span class="ev-time">${esc(when)}</span></span></div>`;
        }).join('');
        h += `</div>${log.total > log.events.length ? `<div class="modal-row" style="justify-content:center;margin:10px 0 2px"><button class="mini" id="ev-more">${t('Load older')}</button></div>` : ''}</div>`;
      }
    }
    if (mySeq !== _actSeq || view.type !== 'activity') return;
    c.innerHTML = h;
    c.querySelectorAll('[data-abandon]').forEach((b) => b.onclick = async () => { if (confirm(t('Abandon this upload? The partial data will be deleted and cannot be resumed.'))) { try { await api('/activity/abandon/' + b.dataset.abandon, { method: 'POST' }); } catch (_) {} draw(); } });
    c.querySelectorAll('[data-resume]').forEach((b) => b.onclick = async () => { b.disabled = true; b.textContent = t('Resuming\u2026'); try { await api('/activity/resume/' + b.dataset.resume, { method: 'POST' }); } catch (_) {} draw(); });
    const _evMore = $('#ev-more'); if (_evMore) _evMore.onclick = () => { _logLimit = Math.min(_logLimit + 50, 200); draw(); };
    const _evClear = $('#ev-clear'); if (_evClear) _evClear.onclick = async () => { if (confirm(t('Clear the entire activity log? This cannot be undone.'))) { try { await api('/activity/log', { method: 'DELETE' }); } catch (_) {} _logLimit = 50; draw(); } };
  };
  await draw();
  _actTimer = setInterval(draw, 1500);
}
...
async function adminGeneral() {
  ...
  $('#set-save').onclick = async () => { const j = {}; const sv = parseInt($('#set-sess').value, 10); if (sv === 0) j.sessionUntilRestart = true; else { j.sessionUntilRestart = false; j.sessionDays = sv; } j.stagingEnabled = $('#set-staging').checked; const _sp = $('#set-staging-path'); if (_sp) j.stagingPath = _sp.value.trim(); const _sg = parseFloat($('#set-staging-gb').value); if (_sg > 0) j.stagingMaxGB = _sg; const _ar = $('#set-autoreload'); if (_ar) j.autoReload = _ar.checked; const _shp = $('#set-shareport'); if (_shp) j.sharePort = parseInt(_shp.value, 10) || 0; const _shu = $('#set-shareurl'); if (_shu) j.sharePublicUrl = _shu.value.trim(); if (isOrg) { j.allowRegistration = $('#set-reg').checked; j.defaultRoleId = $('#set-role').value; j.defaultQuotaMB = parseInt($('#set-quota').value, 10) || 0; } const btn = $('#set-save'); const orig = btn.textContent; btn.disabled = true; btn.textContent = t('Saving…'); $('#set-result').innerHTML = ''; try { await api('/admin/settings', { method: 'PATCH', json: j }); if ((j.sharePort || 0) !== (s.sharePort || 0)) { if (confirm(t('The public-share port changed. TCloud must restart to apply it. Restart now?'))) { btn.textContent = t('Restarting\u2026'); $('#set-result').innerHTML = `<div class="share-hint">${t('Restarting TCloud \u2014 reconnecting when it is back\u2026')}</div>`; try { await api('/admin/restart', { method: 'POST' }); } catch (_) {} waitForServerThenReload(); return; } $('#set-result').innerHTML = `<div class="su-note">${t('Saved. The new public-share port takes effect after the next restart.')}</div>`; btn.textContent = orig; btn.disabled = false; return; } btn.classList.add('saved-ok'); btn.textContent = '✅ ' + t('Saved'); $('#set-result').innerHTML = `<div class="share-hint">\u2705 ${t('Settings saved.')}</div>`; setTimeout(() => { btn.classList.remove('saved-ok'); btn.textContent = orig; btn.disabled = false; $('#set-result').innerHTML = ''; }, 1800); } catch (e) { btn.disabled = false; btn.textContent = orig; $('#set-result').innerHTML = `<div class="login-error">${esc(e.message)}</div>`; } };
